...
class SceneBuilder:
    ...
    def take_object_id(self) -> int:
        object_id = self.next_object_id
        self.next_object_id += 1
        return object_id

    def add_quad(self, triangles, a, b, c, d, material, object_id: int | None = None) -> None:
        if object_id is None:
            object_id = self.take_object_id()
        triangles.append(self.Triangle(a, b, c, material, object_id))
        triangles.append(self.Triangle(a, c, d, material, object_id))
    ...
    def add_box(self, triangles, minimum, maximum, material, object_id: int | None = None) -> None:
        Vec3 = self.Vec3
        if object_id is None:
            object_id = self.take_object_id()
        ...
        self.add_quad(triangles, p000, p001, p011, p010, material, object_id)
        self.add_quad(triangles, p100, p110, p111, p101, material, object_id)
        self.add_quad(triangles, p000, p100, p101, p001, material, object_id)
        self.add_quad(triangles, p010, p011, p111, p110, material, object_id)
        self.add_quad(triangles, p001, p101, p111, p011, material, object_id)
        self.add_quad(triangles, p000, p010, p110, p100, material, object_id)

    def add_pyramid(self, triangles, center, size: float, height: float, material, object_id: int | None = None) -> None:
        Vec3 = self.Vec3
        if object_id is None:
            object_id = self.take_object_id()
        half = size * 0.5
        ...
        self.add_quad(triangles, p0, p1, p2, p3, material, object_id)
        triangles.append(self.Triangle(p0, top, p1, material, object_id))
        triangles.append(self.Triangle(p1, top, p2, material, object_id))
        triangles.append(self.Triangle(p2, top, p3, material, object_id))
        triangles.append(self.Triangle(p3, top, p0, material, object_id))
    ...
    def add_triangle_plate(
        ...
        triangles.append(self.Triangle(p0, p2, p1, material, object_id))

    def add_faceted_sphere(
        self,
        triangles,
        center,
        radius: float,
        ...
        for segment in range(segments):
            ...
            triangles.append(self.Triangle(top, points[0][segment], points[0][next_segment], material, object_id))
        ...
        for ring in range(len(points) - 1):
            ...
            for segment in range(segments):
                next_segment = (segment + 1) % segments
                triangles.append(self.Triangle(upper[segment], lower[segment], lower[next_segment], material, object_id))
                triangles.append(self.Triangle(upper[segment], lower[next_segment], upper[next_segment], material, object_id))

        last = points[-1]
        for segment in range(segments):
            next_segment = (segment + 1) % segments
            triangles.append(self.Triangle(bottom, last[next_segment], last[segment], material, object_id))
